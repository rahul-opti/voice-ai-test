...
from zeta_voice.database import Conversation, ConversationContext, Error, Log, convert_entry_to_dict, get_db
from zeta_voice.database.actions import list_active_call_sids
from zeta_voice.interface.telephony import TwilioClient
...
def get_metrics(
    ...
    metrics.append("# HELP zeta_voice_conversation_status_total Total number of conversations by status.")
    metrics.append("# TYPE zeta_voice_conversation_status_total gauge")
    status_counts = db.query(Conversation.status, func.count(Conversation.status)).group_by(Conversation.status).all()
    for status, count in status_counts:
        metrics.append(f'zeta_voice_conversation_status_total{{status="{status.value}"}} {count}')
    total_conversations = sum(count for _, count in status_counts)
    metrics.append(f'zeta_voice_conversation_status_total{{status="total"}} {total_conversations}')

    metrics.append("")

    # Lead Status Counts
    metrics.append("# HELP zeta_voice_lead_status_total Total number of conversations by lead status.")
    metrics.append("# TYPE zeta_voice_lead_status_total gauge")
    lead_status_counts = (
    ...
    for lead_status, count in lead_status_counts:
        metrics.append(f'zeta_voice_lead_status_total{{lead_status="{lead_status.value}"}} {count}')
    total_leads = sum(count for _, count in lead_status_counts)
    metrics.append(f'zeta_voice_lead_status_total{{lead_status="total"}} {total_leads}')

    metrics.append("")

    # Active Calls
    metrics.append("# HELP zeta_voice_active_calls_total Total number of active calls by source.")
    metrics.append("# TYPE zeta_voice_active_calls_total gauge")
    metrics.append(f'zeta_voice_active_calls_total{{source="database"}} {len(list_active_call_sids(db))}')
    metrics.append(f'zeta_voice_active_calls_total{{source="twilio"}} {len(twilio_client.list_active_calls())}')
