...
from zeta_voice.paths import ELEVENLABS_VOICES_PATH
from zeta_voice.settings import ElevenLabsTTSSettings, settings
from zeta_voice.tts.elevenlabs_tts import async_generate_speech as elevenlabs_async_generate_speech
from zeta_voice.tts.openai_tts import async_generate_speech as openai_async_generate_speech
...
class OpenAITTSService(TTSService):
    """OpenAI TTS service implementation that uses AWS S3."""

    def __init__(self, bot_gender: str | None = None) -> None:
        self.silence_remover = SilenceRemover()
        self.recordings_storage = S3Storage()
        self.base_url = settings.telephony.BASE_URL
        self.static_container_name = settings.storage.AWS_S3_BUCKET_NAME_OAI
...
class ElevenLabsTTSService(TTSService):
    """11labs TTS service implementation that uses AWS S3."""

    def __init__(self, voice_name: str) -> None:
        self.silence_remover = SilenceRemover()
        self.recordings_storage = S3Storage()
        self.base_url = settings.telephony.BASE_URL
        self.static_container_name = settings.storage.AWS_S3_BUCKET_NAME_11LABS
        self.dynamic_container_name = settings.storage.LOCAL_STORAGE_DYNAMIC_CONTAINER_NAME
        self.tts_model = settings.tts.TTS_MODEL
